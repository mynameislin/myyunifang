...
public class FeileiRecyclerView extends RecyclerView.Adapter<FeileiRecyclerView.Item4ViewHolder> {
  private Context context;
    private FenLei fenLei;

    public FeileiRecyclerView(Context context, FenLei fenLei) {
    ...
    public Item4ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                intent.putExtra("id",fenLei.getData().getGoodsBrief().get(position).getId());
    ...
    public void onBindViewHolder(Item4ViewHolder holder, int position) {
          holder.item4_name_tv.setText(fenLei.getData().getGoodsBrief().get(position).getEfficacy());
        holder.item4_title_tv.setText(fenLei.getData().getGoodsBrief().get(position).getGoods_name());
        holder.item4_new_price.setText("￥"+fenLei.getData().getGoodsBrief().get(position).getShop_price());
        holder.item4_price.setText("￥"+fenLei.getData().getGoodsBrief().get(position).getMarket_price());
        holder.item4_price.getPaint().setFlags(Paint.STRIKE_THRU_TEXT_FLAG);
        Glide.with(context).load(fenLei.getData().getGoodsBrief().get(position).getGoods_img()).into(holder.item4_iv);
    }
    @Override
    public int getItemCount() {
        return fenLei.getData().getGoodsBrief().size();
    }
